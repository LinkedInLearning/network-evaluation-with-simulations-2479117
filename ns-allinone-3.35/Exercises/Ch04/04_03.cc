...
/* 
 * Example of Simulating an Ethernet with Trace Files
 ...
 *        ./waf --run="scratch/04_03"
 *        ./waf --run="scratch/04_03 --logging=false"
 *        ./waf --run="scratch/04_03 --nCsma=3 --logging=true"
 */
...
main (int argc, char *argv[])
{
  ...
  // Enabled ASCII and PCAP trace files
  AsciiTraceHelper ascii;
  pointToPoint.EnableAsciiAll (ascii.CreateFileStream ("ethernet.tr"));
  csma.EnableAscii("ethernet-n1.tr", csmaDevices.Get (0), true); 
 ...
 // Calculate the actual throughput
  uint64_t rxBytes = 0;
  uint32_t payloadSize = 1024;   // UDP packet length in bytes
  rxBytes = payloadSize * 1; // 1 packet wasa sent/received in this Echo application
  double simulationTime = 10.0 - 2.0; // The actual simulataion period
  double throughput = (rxBytes * 8) / (simulationTime * 1000000.0);   // Mbps

  Simulator::Destroy();
  NS_LOG_INFO("Done.");

  // Use C++ standard to output information
  std::cout << "Throughput calculated: " << throughput << " Mbps" << std::endl;
